...
var proxy_agent = require('proxy-agent');
...
deployTask.getHandler = function (grunt) {
  ...
  return function () {
    ...
    if (process.env.https_proxy !== undefined) {
      AWS.config.update({
        httpOptions: {
          agent: proxy_agent(process.env.https_proxy)
        }
      });
    }
    ...
    lambda.getFunction({
      ...
      if (options.environment !== null) {
        ...
        configParams.Environment.Variables = options.environment;
      }
      ...
      var getLayersVersion = function(){
        var deferred = Q.defer();
        if(options.layers === null) {
          deferred.resolve();
        } else {
          var params = {
            StackName: options.layers
          };
          cloudformation.describeStacks(params, function(err, data) {
            if (err) {
              grunt.fail.warn('LayerVersion ' + options.layers + ' failed with message ' + err.stack);
              console.log(err, err.stack); // an error occurred
              deferred.reject();
            } else {
              grunt.log.writeln('LayerVersion ' + data.Stacks[0].Outputs[0].OutputValue);
              configParams.Layers = [data.Stacks[0].Outputs[0].OutputValue];
              deferred.resolve();
            }    
          });
        }
